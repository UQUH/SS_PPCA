%% Stage -3 Optimization objective function for NPM
function J_alpha = objective_fun_NPM_stage3(alpha,M, K, beta_coff,...
...
%% load up result of stage 1 and 2
load("alpha_opt_stage1.mat",'alpha_stage_1');
load("alpha_opt_stage2.mat",'alpha_stage_2');
alpha_vec = [alpha_stage_1(3:end), alpha_stage_2];
...
for i = 1:v_sim
    for j = 1:k
        for l = 1:m_dof
            a_j_x = pi.*k_v/(L(1)*alpha);
            a_j_y = pi.*k_v/(L(2)*alpha);
            a_j_z = pi.*k_v/(L(3)*alpha);

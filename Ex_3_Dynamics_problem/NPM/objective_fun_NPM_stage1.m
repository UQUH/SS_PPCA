%% Stage -1 Optimization objective function for NPM
function J_alpha = objective_fun_NPM_stage1(alpha,M, K, beta_coff,...
    sorted_index, F, qoi_HDM, qoi_ROM, V_global, lambda_v, k_v, L, x)
% Initialize Parameters and Random Seed
alpha_mat = diag(alpha(3:end)); % Create diagonal matrix from alpha values
...
for i = 1:v_sim
    for j = 1:k
        for l = 1:m_dof
            a_j_x = pi.*k_v/(L(1)*alpha(2));
            a_j_y = pi.*k_v/(L(2)*alpha(2));
            a_j_z = pi.*k_v/(L(3)*alpha(2));
    ...
    W = (V_global + alpha(1)*Z)*H_s_Z; %stochastic reduced-order basis

%% Stage -4 Optimization objective function for NPM
function J_alpha = objective_fun_NPM_stage4(alpha,M, K, beta_coff,...
...
%% load up stage 3 result
load("alpha_opt_stage3.mat",'alpha_stage_3');

alpha_vec = [alpha(2:end)];
...
for i = 1:v_sim
    for j = 1:k
        for l = 1:m_dof
            a_j_x = pi.*k_v/(L(1)*alpha_stage_3(2));
            a_j_y = pi.*k_v/(L(2)*alpha_stage_3(2));
            a_j_z = pi.*k_v/(L(3)*alpha_stage_3(2));
    ...
    Z = U - V_global*D;
    H_s_Z = (eye(k) + (alpha(1)^2)*Z'*M*Z)^(-0.5);
    W = (V_global + alpha(1)*Z)*H_s_Z;
